    ...
    if request.method == "POST":
        ...
        if purchased_item_object:
            if current_user.can_purchase(purchased_item_object):
                ...
                flash(f"Unfortunately you cannot purchase {purchased_item_object.name}", category="danger")
        ...
        if sold_item_object:
            if current_user.can_sell(sold_item_object):
                ...
                flash(f"Congratulations! You sold {sold_item_object.name} back to the market for ${sold_item_object.price}", category="success")
            else:
